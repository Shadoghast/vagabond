...
export class ProjectRoll extends DSRoll {
    ...
    if (!options.appliedModifier) {
      ...
      if (this.netFav) {
        ...
        const number = new foundry.dice.terms.NumericTerm({
          number: Math.min(4, 2 * Math.abs(this.netFav)),
          flavor: game.i18n.localize(`VAGABOND.Roll.Power.Modifier.${this.netFav > 0 ? "Favor" : "Hinder"}`)
        });
  ...
  static async prompt(options = {}) {
    ...
    const flavor = options.flavor ?? game.i18n.localize("VAGABOND.Roll.Project.Label");
    ...
    const rollContext = await foundry.applications.api.DialogV2.prompt({
      window: {title: "VAGABOND.Roll.Project.Label"},
      content,
      ok: {
