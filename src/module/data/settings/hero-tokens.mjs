...
/**
 * A data model to manage Hero Tokens in Vagabond
 */
export class HeroTokenModel extends foundry.abstract.DataModel {
  ...
  static label = "VAGABOND.Setting.HeroTokens.Label";

  /** Helper text for Hero Tokens */
  static hint = "VAGABOND.Setting.HeroTokens.Hint";
  ...
  async spendToken(spendType, options = {}) {
    if (!game.users.activeGM) {
      ui.notifications.error("VAGABOND.Setting.NoActiveGM", {localize: true});
      return false;
    }
    ...
    if (currentTokens < tokenSpendConfiguration.tokens) {
      ui.notifications.error("VAGABOND.Setting.HeroTokens.NoHeroTokens", {localize: true});
      return false;
    }
