...
export default class SourceModel extends foundry.abstract.DataModel {
  ...
  static getModuleBook(pkg) {
    if (!pkg) return null;
    const sourceBooks = foundry.utils.getProperty(pkg, "flags.vagabond.sourceBooks");
   ...
   * @returns {VagabondActor | VagabondItem}
   */
  get document() {
  ...
  prepareData(uuid) {
    ...
    const page = Number.isNumeric(this.page)
      ? game.i18n.format("VAGABOND.Source.Display.Page", {page: this.page}) : (this.page ?? "");
    this.label = game.i18n.format("VAGABOND.Source.Display.Full", {book: this.book, page}).trim();
  ...
  async updateDialog() {
    ...
    const fd = await foundry.applications.api.DialogV2.prompt({
      ...
      window: {
        title: "VAGABOND.Source.UpdateTitle",
