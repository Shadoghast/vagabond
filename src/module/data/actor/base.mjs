...
export default class BaseActorModel extends foundry.abstract.TypeDataModel {
   ...
   * @param {VagabondCombatant} combatant The combatant representation
   */
   ...
   * @returns {Promise<VagabondChatMessage | null>}
   */
  async rollCharacteristic(characteristic, options = {}) {
    ...
    if (types.length > 1) {
      ...
      type = await foundry.applications.api.DialogV2.wait({
        window: {title: game.i18n.localize("VAGABOND.Roll.Power.ChooseType.Title")},
        content: game.i18n.localize("VAGABOND.Roll.Power.ChooseType.Content"),
        buttons,
        rejectClose: true
      });
    }
    ...
    const flavor = `${game.i18n.localize(`VAGABOND.Actor.characteristics.${characteristic}.full`)} ${game.i18n.localize(PowerRoll.TYPES[type].label)}`;
    const modifiers = {
    ...
    const messageData = {
      speaker: VagabondChatMessage.getSpeaker({actor: this.parent}),
      rolls: powerRolls,
      sound: CONFIG.sounds.dice
    };
    VagabondChatMessage.applyRollMode(messageData, rollMode);
    return VagabondChatMessage.create(messageData);
  }
  ...
  async takeDamage(damage, options = {}) {
    ...
    if (damage === 0) {
      ui.notifications.info("VAGABOND.Actor.DamageNotification.ImmunityReducedToZero", {format: {name: this.parent.name}});
      return this.parent;
    }
