...
import {VagabondActiveEffect, VagabondActor, VagabondChatMessage} from "../../documents/_module.mjs";
...
export default class AbilityModel extends BaseItemModel {
  ...
  static defineSchema() {
    ...
    schema.damageDisplay = new fields.StringField({choices: {
      melee: "VAGABOND.Item.Ability.Keywords.Melee",
      ranged: "VAGABOND.Item.Ability.Keywords.Ranged"
    ...
    const potencySchema = (initialPotency) => {
      const schema = {
        enabled: new fields.BooleanField({label: "VAGABOND.Item.Ability.FIELDS.powerRoll.tier.potency.enabled.label"}),
        characteristic: new fields.StringField({label: "VAGABOND.Item.Ability.FIELDS.powerRoll.tier.potency.characteristic.label"}),
        value: new FormulaField({deterministic: true, initial: initialPotency, label: "VAGABOND.Item.Ability.FIELDS.powerRoll.tier.potency.value.label"})
      };

      // Localize potencySchema - TODO: Update in V13 onces arrays localize inner fields
      Object.entries(schema).forEach(([field, fieldSchema]) => fieldSchema.label = game.i18n.localize(`VAGABOND.Item.Ability.FIELDS.powerRoll.tier.potency.${field}.label`));
    ...
    const powerRollSchema = ({initialPotency}) => {
      ...
      const baseLabel = "VAGABOND.Item.Ability.FIELDS.powerRoll.tier";
  ...
  toPotencyEmbed(potencyData) {
    return game.i18n.format("VAGABOND.Item.Ability.Potency.Embed", {
      characteristic: game.i18n.localize(`VAGABOND.Actor.characteristics.${potencyData.characteristic}.abbreviation`),
  ...
  async toEmbed(config, options = {}) {
    ...
    const context = {
      ...
      config: ds.CONFIG,
      resourceName: this.actor?.system.coreResource.name ?? game.i18n.localize("VAGABOND.Actor.Character.FIELDS.hero.primary.value.label")
    };
   ...
   * @returns {Promise<Array<VagabondChatMessage> | null>}
  ...
  async use(options = {}) {
    ...
    if (this.spend.value || this.spend.text) {
      ...
      const spendGroup = foundry.applications.fields.createFormGroup({
        label: game.i18n.format("VAGABOND.Item.Ability.ConfigureUse.SpendLabel", {
      ...
      configuration = await foundry.applications.api.DialogV2.prompt({
        content,
        window: {
          title: "VAGABOND.Item.Ability.ConfigureUse.Title",
    ...
    if (configuration) {
      if (configuration.spend) {
        ...
        messageData.flavor = game.i18n.format("VAGABOND.Item.Ability.ConfigureUse.SpentFlavor", {
          value: resourceSpend,
          name: coreResource.name
        });
      }
    ...
    if (this.powerRoll.enabled) {
      ...
      VagabondChatMessage.applyRollMode(messageData, rollMode);
      ...
      for (const tierNumber in groupedRolls) {
        ...
        if (damageEffects.length) {
          for (const damageEffect of damageEffects) {
            ...
            const flavor = game.i18n.format("VAGABOND.Item.Ability.DamageFlavor", {type: damageLabel});
        ...
        messages.push(VagabondChatMessage.create(messageDataCopy));
      }

      return Promise.allSettled(messages);
    }
    else return Promise.allSettled([VagabondChatMessage.create(messageData)]);
  }
   ...
   * @param {VagabondActor} target A target of the Ability Roll
  ...
  getTargetModifiers(target) {
    ...
    if (VagabondActiveEffect.isStatusSource(this.actor, target, "frightened")) modifiers.banes += 1; // Attacking the target frightening the actor
    if (VagabondActiveEffect.isStatusSource(target, this.actor, "frightened")) modifiers.edges += 1; // Attacking the target the actor has frightened

    // Grabbed condition check - targeting a non-source adds a bane
    if (VagabondActiveEffect.isStatusSource(this.actor, target, "grabbed") === false) modifiers.banes += 1;
