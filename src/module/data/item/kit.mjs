...
export default class KitModel extends BaseItemModel {
  ...
  async _preCreate(data, options, user) {
    ...
    if (actor) {
      ...
      if (actorClass?.system.kits === 0) {
        const message = game.i18n.format("VAGABOND.Item.Kit.NotAllowedByClass", {class: actorClass.name});
  ...
  async _kitSwapDialog() {
    ...
    let radioButtons = `<strong>${game.i18n.format("VAGABOND.Item.Kit.Swap.Header", {kit: this.parent.name, actor: this.parent.actor.name})}</strong>`;
    ...
    const fd = await foundry.applications.api.DialogV2.prompt({
      ...
      window: {
        ...
        title: "VAGABOND.Item.Kit.Swap.Title"
      },
      position: {
        width: 400
      },
      ok: {
        label: "VAGABOND.Item.Kit.Swap.Button",
        icon: "fa-solid fa-arrow-right-arrow-left",
