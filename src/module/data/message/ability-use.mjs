  ...
  async alterMessageHTML(html) {
    ...
    if (this.embedText) {
      ...
      if (item) {
        ...
        embed.innerText = game.i18n.localize("VAGABOND.Item.Ability.EmbedFail");
      }
      ...
      content.insertAdjacentHTML("afterbegin", `<p class="powerResult"><strong>${
        game.i18n.localize(`VAGABOND.Roll.Power.Results.Tier${tier}`)
  ...
  _constructDamageFooterButtons() {
    ...
    for (const roll of damageRolls) {
      ...
      const button = constructHTMLButton({
        label: game.i18n.format("VAGABOND.Messages.AbilityUse.Buttons.ApplyDamage.Label", {
        ...
        dataset: {
          ...
          amount: roll.total,
          tooltip: game.i18n.localize("VAGABOND.Messages.AbilityUse.Buttons.ApplyDamage.Tooltip"),
          tooltipDirection: "UP"
        },
  ...
  addListeners(html) {
    ...
    for (const damageButton of damageButtons) {
      damageButton.addEventListener("click", async (event) => {
        if (!canvas.tokens.controlled.length) return ui.notifications.error("VAGABOND.Messages.AbilityUse.NoTokenSelected", {localize: true});
