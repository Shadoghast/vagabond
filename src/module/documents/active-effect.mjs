...
export class VagabondActiveEffect extends ActiveEffect {
  ...
  static async targetedConditionPrompt(statusId, effectData) {
    try {
      ...
      ui.notifications.warn("VAGABOND.Effect.TargetedConditionPrompt.Warning", {localize: true});
    }
  }

  /**
   * Determine if the affected actor has the status and if the source is the one imposing it
   * @param {VagabondActor} affected The actor affected by the status
   * @param {VagabondActor} source The actor imposing the status
