...
import VagabondActorSheet from "./base.mjs";
...
export default class VagabondNPCSheet extends VagabondActorSheet {
  ...
  _getEVLabel() {
    ...
    if (this.actor.system.monster.organization === "minion") return game.i18n.format("VAGABOND.Actor.NPC.EVLabel.Minion", data);
    else return game.i18n.format("VAGABOND.Actor.NPC.EVLabel.Other", data);
  }
  ...
  static async _editMonsterMetadata(event, target) {
    ...
    const fd = await foundry.applications.api.DialogV2.prompt({
      ...
      classes: ["vagabond", "monster-metadata"],
      window: {
        title: "VAGABOND.Actor.NPC.MonsterMetadata.DialogTitle",
  ...
  static async _freeStrike(event, target) {
    /** @type {Array<VagabondActor>} */
    const targets = game.user.targets.map(t => t.actor).filter(a => a?.system?.takeDamage).toObject();
    if (!targets.length) {
      ui.notifications.error("VAGABOND.Actor.NPC.FreeStrike.NoTargets", {localize: true});
      return;
    }
    const freeStrike = this.actor.system.freeStrike;

    const damageLabel = game.i18n.format("VAGABOND.Actor.NPC.FreeStrike.DialogHeader", {
    ...
    const fd = await foundry.applications.api.DialogV2.prompt({
      window: {title: "VAGABOND.Actor.NPC.FreeStrike.DialogTitle", icon: "fa-solid fa-burst"},
      content,
      rejectClose: false,
      ok: {
        label: "VAGABOND.Actor.NPC.FreeStrike.DialogButton",
        callback: (event, button, dialog) => new FormDataExtended(button.form)
      }
    });
